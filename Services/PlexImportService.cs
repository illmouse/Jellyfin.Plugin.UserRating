...
namespace Jellyfin.Plugin.UserRatings.Services
{
    public class PlexImportService
    {
        ...
        private readonly ILogger<PlexImportService> _logger;
        private readonly ProgressTracker _progressTracker;

        public PlexImportService(
        ...
        public async Task<ImportResult> ImportFromPlexAsync(Guid jellyfinUserId, string operationId, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                _progressTracker.UpdateProgress(operationId, p => p.Status = "fetching");
                ...
                _logger.LogInformation("Found {MovieLibs} movie libraries and {ShowLibs} show libraries in Plex", movieLibraries.Count, showLibraries.Count);
                ...
                _logger.LogInformation("Found {Count} rated items in Plex to import", totalItems);

                for (var i = 0; i < allRatedPlexItems.Count; i++)
                {
                    if (cancellationToken.IsCancellationRequested)
                    {
                        _progressTracker.UpdateProgress(operationId, p => p.Status = "cancelled");
                        return new ImportResult
                        {
                            Success = false,
                            OperationId = operationId,
                ...
                var (imported, skipped, overwritten) = _repository.BulkSaveRatings(ratings, conflictMode);
                ...
                _progressTracker.CompleteOperation(operationId, result);

                return result;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error importing ratings from Plex");
        ...
        private Guid? ResolvePlexItemToJellyfin(PlexVideo plexItem)
        {
            ...
            if (providerIds.TryGetValue("Imdb", out var imdbId))
            {
                ...
                if (item != null)
                {
                    _logger.LogDebug("Matched '{Title}' via IMDB ID {ImdbId}", plexItem.Title, imdbId);
                    return item.Id;
                }
            }
            ...
            if (providerIds.TryGetValue("Tmdb", out var tmdbId))
            {
                ...
                if (item != null)
                {
                    _logger.LogDebug("Matched '{Title}' via TMDB ID {TmdbId}", plexItem.Title, tmdbId);
                    return item.Id;
                }
            }
            ...
            if (providerIds.TryGetValue("Tvdb", out var tvdbId))
            {
                ...
                if (item != null)
                {
                    _logger.LogDebug("Matched '{Title}' via TVDB ID {TvdbId}", plexItem.Title, tvdbId);
                    return item.Id;
                }
            }

            _logger.LogWarning("Could not match Plex item '{Title}' with GUIDs: {Guids}", plexItem.Title, string.Join(", ", plexItem.Guids.Select(g => g.Id)));
            return null;
        }
        ...
        private MediaBrowser.Controller.Entities.BaseItem? FindByProviderId(string providerKey, string providerValue)
        {
            ...
            {
                _logger.LogDebug(ex, "Error looking up provider ID {Key}={Value}", providerKey, providerValue);
                return null;
            }
        }
